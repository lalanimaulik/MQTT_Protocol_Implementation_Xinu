...
struct topics topicTable[NTOPICS];
struct msgqueue *head;
struct msgqueue *tail;
pid32 bID;
...
static	void	sysinit()
{
	...
	for(i=0;i<NTOPICS;i++){
		initializeTopic = &topicTable[i];			
		int32 j;
		for(j=0;j<8;j++){
			
			initializeTopic-> pids[j] = 0;
			initializeTopic-> handler[j] = 0;
		}
		initializeTopic-> index = 0;			
	}	
	head = NULL;
	tail = NULL;
	return;
